...
Problem = namedtuple("Problem", ['sid', 'model_file', 'input_file', 'name'])
...
def submit():
    ...
    for model in selected_models:
        if not os.path.isfile(model.model_file):
            print 'Unable to locate assignment file "'+metadata.model_file+'" in the current working directory.'
            continue
        ...
        if not login or not ch or not state:
            print '\n!! Error: %s\n' % login
            return
        submission = get_source(model.model_file)
        
        if not isinstance(submission, str):
            print 'Warning: the submitted model file "'+model.model_file+'" was not ASCII and will be converted.  Some information may be lost.'
            print 'Orginal: '
            print submission
            submission = submission.encode('ascii', 'ignore')
        
        print 'For part: '+model.name+' Model'
        print 'Submitting: '+model.model_file
    ...
    for problem in selected_problems:
        if not os.path.isfile(problem.model_file):
            print 'Unable to locate assignment file "'+metadata.model_file+'" in the current working directory.'
            continue
        ...
        if not login or not ch or not state:
            print '\n!! Error: %s\n' % login
            return
        submission = output(problem)

        ch_resp = challenge_response(login, password, ch)
        (result, string) = submit_solution(metadata.url, login, ch_resp, problem.sid, submission, get_source(problem.model_file), state, ch_aux)
...
def run_minizinc(model_file, data_file, time_limit=30000):
    print 'running gecode minizinc as a subprocess on input '+data_file+' with model '+model_file+'...'
    cmd = ['mzn-gecode', model_file, data_file, '--all-solutions']
...
def output(problem):
    ...
    start = time.clock()
    try:
        stdout, stderr = run_minizinc(problem.model_file, problem.input_file, time_limit=3600000)
        #stdout, stderr = run_minizinc(problem.model_file, problem.input_file, time_limit=10000)
    except Exception, e:
        print 'running minizinc as a subprocess on input '+problem.input_file+' with model '+problem.model_file+' raised an exception'
